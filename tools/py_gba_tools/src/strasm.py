...
# argument handling for the program
...
def strasm_interpreter(file_list):
    ...
    # all symbols of strasm elements of strasm file
    symbols = []
    # all strasm elements of strasm file
    pstrings = []
    # all strasm elements in bytes of strasm file
    byteBlockTable = []
    # all types of strasm elements of strasm file
    types = []
    ...
    # convert file list into one string
    for i in range(len(file_list)):
        if(file_list[i] == '\n' or file_list[i].startswith('//')):
            continue
        file_string += file_list[i].strip('\n').strip('\r').lstrip(' ').lstrip('\t')

    try:
        # split all strasm elements of strasm file into keywords for strasm
        new_string = file_string.split(';')
...
# check for errors in strasm file and handle strasm file converting        
...
def generate_c_file(byteBlockTable, symbolTable, stringTable, types):
    ...
    for i in range(len(symbolTable)):
        if(types[i] == data_types[0]):
            ...
            max_str_len = len(max(stringTable[i], key=len))+1
            outfile.write( '#define ' + symbolTable[i].upper() + '_LEN ' + str(len(byteBlockTable[i])) + '\n#define ' + symbolTable[i].upper() + '_ENTRY_LEN ' + str(max_str_len) + '\nextern const unsigned char ' + symbolTable[i] + '[' + str(len(byteBlockTable[i])) + '][' + str(max_str_len) + '];\n\n')
    outfile.write('#endif // ' + symbolTable[i].upper() + '_H\n\n/* --- EOF --- */')
    outfile.close()

# support for asm, but out of date
